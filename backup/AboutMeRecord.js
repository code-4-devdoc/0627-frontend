    ...
    return (
        <SectionContainer title="AboutMe">
            ...
            <div style={{ display: "flex", marginLeft: 39 }}>
                <div>
                    <Input style={{ width: 600, height: 60, fontFamily: "inherit" }} as="textarea"
                           placeholder="자기소개를 입력하세요."
                           disabled={!isActive.selfIntroduction} {...introInput} isValid={introInput.isValid} />
                    {(isActive.selfIntroduction && !introInput.isValid) &&
                        <p style={{ color: 'rgba(202, 5, 5, 1)', marginTop: -8, marginBottom: 7, fontSize: 13 }}>입력을
                            확인해 주세요.</p>}
                </div>
                <Button onClick={() => toggleActive('selfIntroduction', introInput)}
