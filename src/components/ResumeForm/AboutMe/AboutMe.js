...
const AboutMe = ({ aboutMes, setAboutMes, resumeId }) => {
    ...
    const phoneInput = useInputValidation(aboutMes.phoneNumber, /^\d{3}-\d{4}-\d{4}$/);
    const emailInput = useInputValidation(aboutMes.email, /^[a-zA-Z0-9.]+@[a-z]+\.[a-z]+$/);
    const birthdayInput = useInputValidation(aboutMes.birthday, /^\d{4}\.\d{2}\.\d{2}$/);
    const githubInput = useInputValidation(aboutMes.github, /^https:\/\/github\.com\/([a-zA-Z0-9_-]+\/?[a-zA-Z0-9_-]*\/?)*$/);
    const blogInput = useInputValidation(aboutMes.blog, /^(https?:\/\/)?([\da-z\.-]+)\.([a-z\.]{2,6})([\/\w \.-]*)*\/?$/);
    const introInput = useInputValidation(aboutMes.introduction, /^[\s\S]*$/);
